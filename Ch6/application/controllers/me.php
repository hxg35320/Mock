...
public function index() {
	...
	//$data = array( 
    //    'log_users.user_id' => $this->session->userdata('usr_id')); 
         
	$this->Login_model->login_user_update($this->session->userdata);
	
	$data['page_heading'] = 'User Profile';
    $email = $this->session->userdata('usr_email');	
    $data['query'] = $this->Users_model->get_user_details_by_email($email);
    $this->load->view('common/header', $data);
    $this->load->view('nav/top_nav', $data);        
    $this->load->view('users/my_up', $data);
 ...
 public function edit_val() {
    ...
    if ($this->form_validation->run() == FALSE) { // First load, or problem with form
      ...
      if ($this->Users_model->process_update_user($this->session->userdata('usr_id'),$data)) {
      ...
      redirect('users'); 
         ...
         public function me_ud(){
		$data['page_heading'] = 'My Details';
    $email = $this->session->userdata('usr_email');	
    $data['query'] = $this->Users_model->get_user_details_by_email($email);
    $this->load->view('common/header', $data);
    $this->load->view('nav/top_nav', $data);        
    $this->load->view('users/me_ud', $data);
    $this->load->view('common/footer', $data);
